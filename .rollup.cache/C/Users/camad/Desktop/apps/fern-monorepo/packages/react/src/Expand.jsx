...
export default function Expand({ innerRef, id, className, children, onAction, exitOnClick }) {
    const [active, setActive] = useState(false);
    const [contentHeight, setContentHeight] = useState(0);
    const expandRef = innerRef || useRef();
    const setExpandHeight = () => setContentHeight(expandRef.current.querySelector('div').clientHeight);
    ...
    return (<div ref={expandRef} id={id} className={cn('fui-listener fui-expand', className)} style={expandStyle(active, contentHeight)} tabIndex={active ? undefined : -1}>
      <div>
        {children}
      </div>
    </div>);
